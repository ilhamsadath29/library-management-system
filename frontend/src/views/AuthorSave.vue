    ...
    <PageComponent>
        <template v-slot:header>
            <div class="flex justify-between items-center">
                <h1 class="text-3xl font-bold text-gray-900">Author - {{ model.id ? `${model.name} Update` : 'Create'}}</h1>
                <router-link :to="{name: 'Author'}" class="py-2 px-3 text-white bg-emerald-500 rounded-md hover:bg-emerald-600">
        ...
        <div >
            ...
            <form v-else class="mt-8 space-y-6" @submit.prevent="saveData">
                ...
                <div class="rounded-md shadow-sm -space-y-px">
                    <div class="pb-2">
                        <label for="name pb-2">Author Name</label>
                    </div>
                    <div class="pb-2">
                        <input
                            id="name"
...
// handle edit page
if (route.params.id) {
    store.dispatch("getAuthor", route.params.id).then((data) => {
...
function saveData() {
    ...
    if(model.value.site_id == null) {
        ...
        store.dispatch("saveAuthor", model.value).then((data) => {
            store.commit("notify", {
                type: "Success",
